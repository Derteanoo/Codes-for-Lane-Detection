...
def test_lanenet(image_path, weights_path, use_gpu, image_list, batch_size, net_flag):
    """
    ...
    with sess.as_default():
        ...
        for i in range(int(len(image_list) / batch_size)):
            ...
            for cnt in range(batch_size):
                ...
                directory = 'predicts_SCNN_test_final/'+ net_flag + image_prefix
                print('directory=',directory)
                if not os.path.exists(directory):
                    os.makedirs(directory)
                file_exist = open(directory + image_name[-10:-4] + '.exist.txt', 'w')

                read_dir= '/home/zdw/SCNN/data/CULane'
                img=cv2.imread(read_dir+image_name)
                img = cv2.resize(img, (800, 288), interpolation=cv2.INTER_CUBIC)
                seg=(instance_seg_image[cnt, :, :, 1] * 255).astype(int)
                ...
                seg=(instance_seg_image[cnt, :, :, 2] * 255).astype(int)
                for i in range(len(seg)):
                    for j in range(len(seg[0])):
                        if seg[i][j] > 50:
                            img[i,j,0]=0
                ...
                seg=(instance_seg_image[cnt, :, :, 3] * 255).astype(int)
                for i in range(len(seg)):
                    for j in range(len(seg[0])):
                        if seg[i][j] > 50:
                            img[i,j,0]=0
                ...
                seg=(instance_seg_image[cnt, :, :, 4] * 255).astype(int)
                for i in range(len(seg)):
                    for j in range(len(seg[0])):
                        if seg[i][j] > 50:
                            img[i,j,0]=0
                ...
                cv2.imwrite(directory + image_name[-10:-4] + '_avg.png', img )   

                for cnt_img in range(4):
                    file_exist.write(str(existence_output[cnt, cnt_img]) + ' ')    
...
if __name__ == '__main__':
    ...
    test_lanenet(args.image_path, args.weights_path, args.use_gpu, img_name, args.batch_size, args.net)
