...
"""
import sys
sys.path.insert(0,'/home/zdw/Codes-for-Lane-Detection/SCNN-Tensorflow/lane-detection-model')
...
def train_net(dataset_dir, weights_path=None, net_flag='vgg'):
    ...
    # Set tf summary
    tboard_save_path = 'tboard/culane/{:s}'.format(net_flag)
    if not ops.exists(tboard_save_path):
        os.makedirs(tboard_save_path)

    total_loss_scalar=tf.summary.scalar('total_loss',total_loss)
    instance_loss_scalar=tf.summary.scalar('instance_loss',instance_loss)
    existence_loss_scalar=tf.summary.scalar('existence_loss',existence_loss)
    accuracy_scalar=tf.summary.scalar('accuracy',accuracy)
    accuracy_back_scalar=tf.summary.scalar('accuracy_back',accuracy_back)
    IoU_scalar=tf.summary.scalar('IoU',IoU)
    learning_rate_scalar=tf.summary.scalar('learning_rate',learning_rate)

    val_total_loss_scalar=tf.summary.scalar('val_total_loss',total_loss)
    val_instance_loss_scalar=tf.summary.scalar('val_instance_loss',instance_loss)
    val_existence_loss_scalar=tf.summary.scalar('val_existence_loss',existence_loss)
    val_accuracy_scalar=tf.summary.scalar('val_accuracy',accuracy)
    val_accuracy_back_scalar=tf.summary.scalar('val_accuracy_back',accuracy_back)
    val_IoU_scalar=tf.summary.scalar('val_IoU',IoU)
    
    train_merge_summary_op=tf.summary.merge([total_loss_scalar,instance_loss_scalar,existence_loss_scalar,
                                    accuracy_scalar,accuracy_back_scalar,learning_rate_scalar,IoU_scalar])
    val_merge_summary_op=tf.summary.merge([val_total_loss_scalar,val_instance_loss_scalar,
                           val_existence_loss_scalar,val_accuracy_scalar,val_accuracy_back_scalar,val_IoU_scalar])
    ...
    sess_config = tf.ConfigProto(device_count={'GPU': 1}) # device_count={'GPU': 1}
    ...
    summary_writer=tf.summary.FileWriter(logdir=tboard_save_path,graph=sess.graph)
    ...
    with sess.as_default():
        ...
        for epoch in range(train_epochs):
            ...
            _, train_summary,c, train_accuracy, train_accuracy_back, train_instance_loss, train_existence_loss, binary_seg_img = \
                sess.run([optimizer, train_merge_summary_op,total_loss,
                          accuracy, 
                          accuracy_back,
                          instance_loss,
                          existence_loss,
            ...
            '''if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                print('Epoch: {:d} loss_ins= {:6f}  loss_ext= {:6f}  accuracy= {:6f}  accuracy_back= {:6f} '
                         ' mean_time= {:5f}s '.
                         format(epoch + 1, train_instance_loss, train_existence_loss,  train_accuracy, train_accuracy_back, cost_time))'''
            ...
            val_summary,c_val, val_accuracy, val_accuracy_back, val_IoU, val_instance_loss, val_existence_loss  = \
                  sess.run([val_merge_summary_op,total_loss, accuracy, accuracy_back, IoU, instance_loss, existence_loss],
            ...
            '''if epoch % CFG.TRAIN.TEST_DISPLAY_STEP == 0:
                print('Epoch_Val: {:d} loss_ins= {:6f} ({:6f}) '
                             ...
                             'mean_time= {:5f}s '.
                             format(epoch + 1, val_instance_loss, np.mean(val_instance_loss_mean), val_existence_loss, np.mean(val_existence_loss_mean), val_accuracy, np.mean(val_accuracy_mean), val_accuracy_back, np.mean(val_accuracy_back_mean), val_IoU, np.mean(val_IoU_mean), np.mean(val_cost_time_mean)))'''

            '''if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                print('Epoch_Val: {:d} loss_ins= {:6f}  '
                             'loss_ext= {:6f}  accuracy= {:6f}  accuracy_back= {:6f}  mIoU= {:6f} '
                             'mean_time= {:5f}s '.
                             format(epoch + 1, val_instance_loss , val_existence_loss,  val_accuracy, val_accuracy_back, val_IoU, cost_time_val))'''
