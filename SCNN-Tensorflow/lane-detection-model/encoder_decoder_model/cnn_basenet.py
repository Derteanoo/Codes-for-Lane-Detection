...
class CNNBaseModel(object):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME',
        ...
        with tf.variable_scope(name):
            ...
            padding = padding.upper()#将小写字母转为大写
    ...
    @staticmethod
    def maxpooling(inputdata, kernel_size, stride=None, padding='SAME',
    ...
    @staticmethod
    def avgpooling(inputdata, kernel_size, stride=None, padding='SAME',
                   data_format='NHWC', name=None):
        """
    ...
    @staticmethod
    def depthwise_conv2d(input_tensor, name=None, k_size=3, stride=1, padding='SAME', out_channel=1,
                         w_init=None, b_init=None ,use_bias=False):
        with tf.variable_scope(name):
            in_shape = input_tensor.get_shape().as_list()
            in_channel = in_shape[3]
            assert in_channel is not None, "[Conv2D] Input cannot have unknown channel!"
            ...
            if isinstance(k_size, list):
                filter_shape = [k_size[0], k_size[1]] + [in_channel, out_channel]
            else:
                filter_shape = [k_size, k_size] + [in_channel, out_channel]
            ...
            if use_bias:
                b = tf.get_variable('b', [in_channel], initializer=b_init)

            conv = tf.nn.depthwise_conv2d(input_tensor, w, strides=strides,
                                padding=padding, rate=[1, 1])
