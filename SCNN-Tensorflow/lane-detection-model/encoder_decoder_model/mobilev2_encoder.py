...
class MOBILENETV2Encoder(cnn_basenet.CNNBaseModel):
    """
    实现了一个基于MOBILENETV2的特征编码类
    """
    ...
    def encode(self, input_tensor, name):
        """
        根据MOBILENETV2框架对输入的tensor进行编码
        :param input_tensor:
        :param name:
        :param flags:
        :return: 输出MOBILENETV2编码特征
        """
        ret = OrderedDict()
        ...
        with tf.variable_scope(name):
            ...
            # bottleneck_conv stage 2
            conv_2_1_bottleneck = self.res_block(input_tensor=conv_1, expansion_ratio=1, out_dims=16,
                                         name='conv_2_1_bottleneck')#(8,144,400,16)         
            
            # bottleneck_conv stage 3
            conv_3_1_bottleneck = self.res_block(input_tensor=conv_2_1_bottleneck, expansion_ratio=6, out_dims=24,
                                         name='conv_3_1_bottleneck', stride=2)#(8,72,200,24) 
            
            conv_3_2_bottleneck = self.res_block(input_tensor=conv_3_1_bottleneck, expansion_ratio=6, out_dims=24,
                                         name='conv_3_2_bottleneck')#(8,72,200,24)  
            
            # bottleneck_conv stage 4
            conv_4_1_bottleneck = self.res_block(input_tensor=conv_3_2_bottleneck, expansion_ratio=6, out_dims=32,
                                         name='conv_4_1_bottleneck', stride=2)#(8,36,100,32) 
            
            conv_4_2_bottleneck = self.res_block(input_tensor=conv_4_1_bottleneck, expansion_ratio=6, out_dims=32,
                                         name='conv_4_2_bottleneck')#(8,36,100,32)   

            conv_4_3_bottleneck = self.res_block(input_tensor=conv_4_2_bottleneck, expansion_ratio=6, out_dims=32,
                                         name='conv_4_3_bottleneck')#(8,36,100,32)                                           
            print("conv_4_3_bottleneck=",conv_4_3_bottleneck)

            # bottleneck_conv stage 5
            conv_5_1_bottleneck = self.res_block(input_tensor=conv_4_3_bottleneck, expansion_ratio=6, out_dims=64,
                                         name='conv_5_1_bottleneck')#(8,36,100,64) 
            
            conv_5_2_bottleneck = self.res_block(input_tensor=conv_5_1_bottleneck, expansion_ratio=6, out_dims=64,
                                         name='conv_5_2_bottleneck')#(8,36,100,64)  

            conv_5_3_bottleneck = self.res_block(input_tensor=conv_5_2_bottleneck, expansion_ratio=6, out_dims=64,
                                         name='conv_5_3_bottleneck')#(8,36,100,64)   
            
            conv_5_4_bottleneck = self.res_block(input_tensor=conv_5_3_bottleneck, expansion_ratio=6, out_dims=64,
                                         name='conv_5_4_bottleneck')#(8,36,100,64)                                                                            
            print("conv_5_4_bottleneck=",conv_5_4_bottleneck)

            # bottleneck_conv stage 6
            conv_6_1_bottleneck = self.res_block(input_tensor=conv_5_4_bottleneck, expansion_ratio=6, out_dims=96,
                                         name='conv_6_1_bottleneck', stride=2)#(8,18,50,96) 
            
            conv_6_2_bottleneck = self.res_block(input_tensor=conv_6_1_bottleneck, expansion_ratio=6, out_dims=96,
                                         name='conv_6_2_bottleneck')#(8,18,50,96)  

            conv_6_3_bottleneck = self.res_block(input_tensor=conv_6_2_bottleneck, expansion_ratio=6, out_dims=96,
                                         name='conv_6_3_bottleneck')#(8,18,50,96)   
                                                                                     
            print("conv_6_3_bottleneck=",conv_6_3_bottleneck)            

            ### add dilated convolution ###

            # conv stage 7_1
            conv_7_1 = self._conv_dilated_stage(input_tensor=conv_6_3_bottleneck, k_size=3,
...
if __name__ == '__main__':
    ...
    encoder = MOBILENETV2Encoder(phase=tf.constant('train', dtype=tf.string))
