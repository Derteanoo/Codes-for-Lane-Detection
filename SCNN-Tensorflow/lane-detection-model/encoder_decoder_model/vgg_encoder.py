    ...
    def encode(self, input_tensor, name):
        ...
        with tf.variable_scope(name):
            ...
            for cnt in range(conv_5_5.get_shape().as_list()[1]):
                feature_list_old.append(tf.expand_dims(conv_5_5[:, cnt, :, :], axis=1))#把conv_5_5从H维从高到低压入feature_list_old
                #print("feature_list_old=",feature_list_old)
            feature_list_new.append(tf.expand_dims(conv_5_5[:, 0, :, :], axis=1))#把conv_5_5从H维第1维压入feature_list_old
            print("feature_list_new=",feature_list_new)#(8, 1, 100, 128)
            ...
            with tf.variable_scope("convs_6_1"):
                conv_6_1 = tf.add(tf.nn.relu(tf.nn.conv2d(feature_list_old[0], w1, [1, 1, 1, 1], 'SAME')), feature_list_old[1])#conv、relu后与前一个feature map相加
                print("conv_6_1=",conv_6_1)#(8, 1, 100, 128)
            ...
            conv_output = self.conv2d(inputdata=dropout_output, out_channel=5,
                               kernel_size=1, use_bias=True, name='conv_6')#(8, 36, 100, 5)
            ...
            reshape_output = tf.reshape(avg_pool, [N, -1])  #(8, 4500)
