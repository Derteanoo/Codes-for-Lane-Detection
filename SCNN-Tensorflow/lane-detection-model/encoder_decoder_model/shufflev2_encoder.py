...
class SHUFFLENETV2Encoder(cnn_basenet.CNNBaseModel):
    """
    实现了一个基于SHUFFLENETV2的特征编码类
    """
    ...
    def shufflev2_block(self, input_tensor, out_dims, name, num_groups=2, stride=1, pad='SAME',
                           k_size=3, use_split=True, use_shuffle=True):#stride,ksize,depthwise
        with tf.variable_scope(name):
            half_channel = out_dims // 2

            if use_split == False:
                if use_shuffle == True:
                    shuffled = self.channel_shuffle(name = 'channel_shuffle', x = input_tensor, num_groups = num_groups)
                else:
                    shuffled = input_tensor
 
                dw_conv_1 = self.depthwise_conv2d(input_tensor=shuffled, name='dw_conv_1',
                               k_size=k_size, stride=stride, padding=pad)

                dw_bn_1 = self.layerbn(inputdata=dw_conv_1, is_training=self._is_training, name='dw_bn_1')  

                pw_conv_1 = self.conv2d(inputdata=dw_bn_1, out_channel=half_channel,
                            kernel_size=1, stride=1, use_bias=False, padding=pad, name='pw_conv_1')
       
                pw_bn_1 = self.layerbn(inputdata=pw_conv_1, is_training=self._is_training, name='pw_bn_1')  
                
                relu_1 = self.relu(inputdata=pw_bn_1, name='relu_1')

                pw_conv_2_1 = self.conv2d(inputdata=shuffled, out_channel=half_channel,
                            kernel_size=1, stride=1, use_bias=False, padding=pad, name='pw_conv_2_1')
      
                pw_bn_2_1 = self.layerbn(inputdata=pw_conv_2_1, is_training=self._is_training, name='pw_bn_2_1') 

                relu_2 = self.relu(inputdata=pw_bn_2_1, name='relu_2')  

                dw_conv_2 = self.depthwise_conv2d(input_tensor=relu_2, name='dw_conv_2',
                               k_size=k_size, stride=stride, padding=pad)
  
                dw_bn_2 = self.layerbn(inputdata=dw_conv_2, is_training=self._is_training, name='dw_bn_2')  

                pw_conv_2_2 = self.conv2d(inputdata=dw_bn_2, out_channel=half_channel,
                            kernel_size=1, stride=1, use_bias=False, padding=pad, name='pw_conv_2_2')

                pw_bn_2_2 = self.layerbn(inputdata=pw_conv_2_2, is_training=self._is_training, name='pw_bn_2_2')  
                
                relu_3 = self.relu(inputdata=pw_bn_2_2, name='relu_3')  
                
                concat = tf.concat([relu_1, relu_3], axis=-1)  
 
            else:
                shuffled = self.channel_shuffle(name = 'channel_shuffle', x = input_tensor, num_groups = num_groups)

                top, bottom = tf.split(shuffled, num_or_size_splits=2, axis=3)

                pw_conv_1 = self.conv2d(inputdata=top, out_channel=half_channel,
                            kernel_size=1, stride=1, use_bias=False, padding=pad, name='pw_conv_1')

                pw_bn_1 = self.layerbn(inputdata=pw_conv_1, is_training=self._is_training, name='pw_bn_1') 

                relu_2 = self.relu(inputdata=pw_bn_1, name='relu_2')  

                dw_conv_1 = self.depthwise_conv2d(input_tensor=relu_2, name='dw_conv_1',
                               k_size=k_size, stride=stride, padding=pad)

                dw_bn_1 = self.layerbn(inputdata=dw_conv_1, is_training=self._is_training, name='dw_bn_1')  

                pw_conv_2 = self.conv2d(inputdata=dw_bn_1, out_channel=half_channel,
                            kernel_size=1, stride=1, use_bias=False, padding=pad, name='pw_conv_2')
            
                pw_bn_2 = self.layerbn(inputdata=pw_conv_2, is_training=self._is_training, name='pw_bn_2')  
                
                relu_2 = self.relu(inputdata=pw_bn_2, name='relu_2')  
                
                concat = tf.concat([bottom, relu_2], axis=-1)        
    ...
    def encode(self, input_tensor, name):
        """
        根据SHUFFLENETV2框架对输入的tensor进行编码
        :param input_tensor:
        :param name:
        :param flags:
        :return: 输出SHUFFLENETV2编码特征
        """
        ret = OrderedDict()
        ...
        with tf.variable_scope(name):
            ...
            pool_1 = self.maxpooling(conv_1, kernel_size=3, stride=2, name='pool_1')#(8, 72, 200, 24)
            print("pool_1=",pool_1)

            # shuffle stage 2
            conv_1_1_shuffle = self.shufflev2_block(input_tensor=pool_1, out_dims=58*2, name='conv_1_1_shuffle',
                              stride=1, pad='SAME', k_size=3, use_split=False, use_shuffle=False)
            print("conv_1_1_shuffle=",conv_1_1_shuffle)
            conv_1_2_shuffle = self.shufflev2_block(input_tensor=conv_1_1_shuffle, out_dims=58*2, name='conv_1_2_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)
            print("conv_1_2_shuffle=",conv_1_2_shuffle)
            conv_1_3_shuffle = self.shufflev2_block(input_tensor=conv_1_2_shuffle, out_dims=58*2, name='conv_1_3_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)
            print("conv_1_3_shuffle=",conv_1_3_shuffle)
            conv_1_4_shuffle = self.shufflev2_block(input_tensor=conv_1_3_shuffle, out_dims=58*2, name='conv_1_4_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)#shuffle3
            print("conv_1_4_shuffle=",conv_1_4_shuffle)

            # shuffle stage 3
            conv_2_1_shuffle = self.shufflev2_block(input_tensor=conv_1_4_shuffle, out_dims=116*2, name='conv_2_1_shuffle',
                              stride=2, pad='SAME', k_size=3, use_split=False)

            conv_2_2_shuffle = self.shufflev2_block(input_tensor=conv_2_1_shuffle, out_dims=116*2, name='conv_2_2_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)

            conv_2_3_shuffle = self.shufflev2_block(input_tensor=conv_2_2_shuffle, out_dims=116*2, name='conv_2_3_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)

            conv_2_4_shuffle = self.shufflev2_block(input_tensor=conv_2_3_shuffle, out_dims=116*2, name='conv_2_4_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)#

            conv_2_5_shuffle = self.shufflev2_block(input_tensor=conv_2_4_shuffle, out_dims=116*2, name='conv_2_5_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)#8

            conv_2_6_shuffle = self.shufflev2_block(input_tensor=conv_2_5_shuffle, out_dims=116*2, name='conv_2_6_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)#9

            conv_2_7_shuffle = self.shufflev2_block(input_tensor=conv_2_6_shuffle, out_dims=116*2, name='conv_2_7_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)#10

            conv_2_8_shuffle = self.shufflev2_block(input_tensor=conv_2_7_shuffle, out_dims=116*2, name='conv_2_8_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)#11
            print("conv_2_8_shuffle=",conv_2_8_shuffle)

            # shuffle stage 4
            conv_3_1_shuffle = self.shufflev2_block(input_tensor=conv_2_8_shuffle, out_dims=232*2, name='conv_3_1_shuffle',
                              stride=2, pad='SAME', k_size=3, use_split=False)

            conv_3_2_shuffle = self.shufflev2_block(input_tensor=conv_3_1_shuffle, out_dims=232*2, name='conv_3_2_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)#13

            conv_3_3_shuffle = self.shufflev2_block(input_tensor=conv_3_2_shuffle, out_dims=232*2, name='conv_3_3_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)

            conv_3_4_shuffle = self.shufflev2_block(input_tensor=conv_3_3_shuffle, out_dims=232*2, name='conv_3_4_shuffle',
                              num_groups=2, stride=1, pad='SAME', k_size=3)
            print("conv_3_4_shuffle=",conv_3_4_shuffle)
...
if __name__ == '__main__':
    ...
    encoder = SHUFFLENETV2Encoder(phase=tf.constant('train', dtype=tf.string))
